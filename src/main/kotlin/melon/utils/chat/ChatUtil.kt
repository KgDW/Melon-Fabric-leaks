...
import dev.zenhao.melon.mixins.IChatHud
            ...
            if (tempMsg == message) {
                ...
                MinecraftClient.getInstance().inGameHud?.let { gameHUD ->
                    val text = Text.literal(
                        "${bracketBuilder(AQUA + Melon.MOD_NAME)} ${
                            message.replace(
                                "§",
                                SECTIONSIGN
                            )
                ...
                MinecraftClient.getInstance().inGameHud?.let { gameHUD ->
                    val text =
                        Text.literal("${bracketBuilder(AQUA + Melon.MOD_NAME)} ${message.replace("§", SECTIONSIGN)}")
                    gameHUD.chatHud?.addMessage(text)
                }
            }
        } ?: {
            MinecraftClient.getInstance().inGameHud?.let { gameHUD ->
                val text = Text.literal("${bracketBuilder(AQUA + Melon.MOD_NAME)} ${message.replace("§", SECTIONSIGN)}")
                gameHUD.chatHud?.addMessage(text)
            }
        }
        tempMsg = message
