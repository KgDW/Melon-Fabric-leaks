...
class Glyph(val char: Char, val font: Font, fallbackFont: Font) {
    val imageTex = Texture("${Melon.DIRECTORY}/glyphs/${font.name.lowercase(Locale.getDefault()).hashCode()}-${char.code}")

    private val finalFont = if (font.canDisplay(char.code)) font else fallbackFont
    val dimensions: Rectangle2D = finalFont.getStringBounds(char.toString(), FontRenderContext(AffineTransform(), true, true))
